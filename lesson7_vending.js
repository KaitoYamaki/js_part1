...
class VendingMachine {
  ...
  addItem(item, num) {
    const items = this.items.filter(function(_item) { return _item.productName === item.productName });
    const cashes = items.map(function(_item) { return _item.cash  });
    if(cashes[0] === item.cash || cashes.length === 0 ) 
    for(let i = 0; i < num; i++) {
  ...
  canBuy(productName) {
    const items = this.findItemList(productName);
    return Boolean(items.includes(items[0]));
  }
}
